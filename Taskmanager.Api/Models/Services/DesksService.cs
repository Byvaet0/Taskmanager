    ...
    public class DesksService : AbstractionService, ICommonService<DeskModel>
    {
        ...
        public bool Update(int id, DeskModel model)
        {
            ...
            bool result = DoAction(delegate ()
            {
                ...
                desk.Columns = JsonConvert.SerializeObject(model.Columns); //из массива строк получаем формат JSON
        ...
        {
            return _db.Desks.Where(d => (d.ProjectId == projectId && (d.AdminId == userId || d.IsPrivate == false))).Select(d => d.ToDto() as CommonModel);
        }
